...
namespace eAgenda.Infra.Orm.ModuloConsulta
{
    public class RepositorioConsultaOrm : RepositorioBase<Consulta>, IRepositorioConsulta
    {
        ...
        public async Task<bool> ExisteConsultaNesseHorarioPorMedicoId(Guid medicoId, TimeSpan horaInicio, TimeSpan horaTermino, DateTime data, Guid? consultaIdIgnorar = null)
        {
            return await registros.AnyAsync(x => x.MedicoId == medicoId &&
                x.Id != consultaIdIgnorar && 
